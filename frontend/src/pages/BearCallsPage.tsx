...
import {
  ...
  softCloseBearCallSpreadMutation,
  forceCloseBearCallSpreadMutation,
...
// Ex-dividend date is the bear-call-specific assignment risk: a deep-ITM short call can be
// assigned the day before ex-div. Highlight when the date is imminent (≤ 3 days out).
function ExDivCell({ exDividendDate }: { exDividendDate: string | null | undefined }) {
  if (!exDividendDate) return <td className="px-3 py-2 text-muted-foreground tabular-nums">—</td>
  const days = dte(exDividendDate.toString())
  const imminent = days >= 0 && days <= 3
  return (
    <td className={`px-3 py-2 tabular-nums ${imminent ? 'text-amber-500 font-medium' : 'text-muted-foreground'}`}>
      {exDividendDate.toString()}{imminent ? ` (${days}d ⚠)` : ''}
    </td>
  )
}

function BearCallRow({ spread }: { spread: BearCallSpreadDto }) {
  const qc = useQueryClient()
  const [confirming, setConfirming] = useState(false)

  const softClose = useMutation({
    ...softCloseBearCallSpreadMutation(),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['listBearCallSpreads'] }),
  })
  const forceClose = useMutation({
    ...forceCloseBearCallSpreadMutation(),
    onSuccess: () => {
      setConfirming(false)
      qc.invalidateQueries({ queryKey: ['listBearCallSpreads'] })
    },
  })
  ...
  return (
    <tr className="border-b border-border hover:bg-muted/40 transition-colors">
      ...
      <ExDivCell exDividendDate={spread.exDividendDate} />
...
export function BearCallsPage() {
  ...
  const paged = data as PagedBearCallSpreadsDto | undefined
  const spreads = paged?.content ?? []
  const totalPages = paged?.totalPages ?? 0
  const totalElements = paged?.totalElements ?? 0

  const sortedSpreads = sorted(spreads as BearCallSpreadDto[], sort, (s, k) => {
    if (k === 'dte') return s.expiryDate ? dte(s.expiryDate.toString()) : null
    if (k === 'exDividendDate') return s.exDividendDate ? new Date(s.exDividendDate).getTime() : null
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold mb-4">Bear Calls</h1>
      ...
      {isError && <p className="text-destructive text-sm">Failed to load bear call spreads.</p>}

      {!isLoading && !isError && spreads.length === 0 && (
        <p className="text-muted-foreground text-sm">No bear call spreads found.</p>
      )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/50 text-muted-foreground text-xs uppercase tracking-wide">
                  ...
                  <SortTh label="Ex-Div" col="exDividendDate" sort={sort} onSort={toggle} className={thClass} />
              ...
              <tbody>
                {sortedSpreads.map((spread) => (
                  <BearCallRow key={spread.id} spread={spread} />
                ))}
              </tbody>
            </table>
          </div>
