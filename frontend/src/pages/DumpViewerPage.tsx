...
async function fetchCoroutineDump() {
    const direct = await fetch('/api/actuator/coroutines')
    if (direct.ok) return direct.text()

    const combined = await fetch('/api/actuator/coroutinedump')
    if (!combined.ok) throw new Error(`Coroutine dump HTTP ${direct.status}/${combined.status}`)
    const text = await combined.text()
    return text.split('---- Coroutine dump ----')[1]?.split('---- Thread dump ----')[0]?.trim() ?? text
}
...
function FindingCard({ f }: { f: Finding }) {
    return (
        <div className={`rounded border px-3 py-2 ${SEVERITY_STYLE[f.severity]}`}>
            ...
            {f.threads.length > 0 && <div className="text-xs text-muted-foreground mt-2 font-mono">{f.threads.join(', ')}</div>}
        </div>
    )
}
    ...
    return (
        <>
            <tr className="border-b last:border-0 hover:bg-muted/40 cursor-pointer" onClick={() => setOpen(!open)}>
                <td className="py-1.5 font-mono text-xs">{g ? <Hint text={g.meaning}>{t.threadName}</Hint> : t.threadName}</td>
...
export function DumpViewerPage() {
    ...
    const coroutineBody = coroutines.trim()
    ...
    return (
        <div className="p-6 space-y-4">
            <div className="flex items-center justify-between gap-3">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        Thread and coroutine dumps, scheduler liveness, and known failure patterns.
                    </p>
                </div>
                <button
                    className="inline-flex items-center gap-2 px-3 py-1.5 rounded bg-primary text-primary-foreground text-sm disabled:opacity-50"
                    onClick={capture}
                    disabled={loading}
                >
            ...
            <section className="space-y-2">
                <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">What looks wrong</h2>
                {findings.length === 0 ? (
                    <div className="rounded border border-dashed px-4 py-6 text-center text-sm text-muted-foreground">
                        No known failure pattern detected. That is not a clean bill of health; it only means none of the specific
                        problems this page knows about are present.
                    </div>
                ) : (
            ...
            <section className="space-y-2">
                ...
                {coroutineEmpty ? (
                    <div className="rounded border border-amber-500/50 bg-amber-500/10 px-3 py-2 text-sm">
                        <strong>The coroutine dump reported nothing; do not read this as healthy.</strong> As of 2026-07-28 it comes
                        back empty even when a coroutine is provably suspended, so it currently proves nothing either way. Trust the
                        thread dump above until this is fixed.
                    </div>
                ) : (
