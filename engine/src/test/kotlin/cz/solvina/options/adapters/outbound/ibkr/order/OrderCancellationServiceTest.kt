...
class OrderCancellationServiceTest {
    ...
    @Test
    fun `atomic cancel verifies order state before issuing cancel`() = runTest {
        val orderId = 123
    ...
    @Test
    fun `stale order already filled before cancel is skipped`() = runTest {
        val orderId = 456
    ...
    @Test
    fun `cancel with verification loop ensures confirmation`() = runTest {
        val orderId = 789
        val openOrder = OpenOrder(
            orderId = orderId,
            symbol = "MSFT",
    ...
    @Test
    fun `cancel timeout after 5 attempts returns failure`() = runTest {
        val orderId = 999
        val openOrder = OpenOrder(
            orderId = orderId,
            symbol = "GOOG",
    ...
    @Test
    fun `empty order list cancels nothing`() = runTest {
    ...
    @Test
    fun `multiple orders cancelled atomically`() = runTest {
