...
class Phase2IntegrationTest {
    ...
    @Test
    fun `issue 5 - multiple stale orders cancelled atomically`() = runTest {
        setup()

        // Simulates multiple stale orders from failed operations
        val staleBuyOrder = OpenOrder(
            orderId = 301,
            symbol = "QQQ",
            action = "BUY",
            orderType = "LMT",
        ...
        val results = orderCancellationService.cancelOrdersAtomic(
            listOf(301, 302),
            "test_multiple",
        )
    ...
    @Test
    fun `issue 5 - already filled orders skipped`() = runTest {
        setup()

        val filledOrder = OpenOrder(
            orderId = 401,
            symbol = "XYZ",
            action = "SELL",
            orderType = "LMT",
    ...
    @Test
    fun `issue 3 - order replacement verifies old order removal`() = runTest {
        setup()

        val orderId = 200
        val existingOrder = OpenOrder(
            orderId = orderId,
            symbol = "MSFT",
            action = "SELL",
            orderType = "LMT",
    ...
    @Test
    fun `issue 3 - order replacement fails gracefully on verification timeout`() = runTest {
        setup()
    ...
    @Test
    fun `issue 5 - partial cancellation on mixed status orders`() = runTest {
        setup()

        val pendingOrder = OpenOrder(
            orderId = 501,
            symbol = "AMD",
            action = "SELL",
            orderType = "LMT",
